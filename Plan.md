...
3. **Docker** : Гарантирует,что приложение будет работать одинаково на разных устройствах и операционных системах.Разработчики могут вместе работать над одним и тем же приложением без возникновений конфликтов между ОС и ПО,ведь докер может размещаться не только на Linux,но и на Windows и MacOS.
4. **Gradle** : В Gradle можно не только задавать зависимости между модулями,но и невероятно гибко описывать зависимости от задач,модулей,каталогов и т.д.Например,в Gradle уже можно сказать,что один модуль зависит от скомпилированных классов,а не от результата(jar) сборки другого.
5. **GitHub** : Обеспечивает плавный и непрерывный процесс внесения изменений в код и не прибегает к громоздкому и неудобному механизму блокировки файлов.
6. **JUnit** : Позволяет параллельно запускать несколько тестов или объединять разные тестовые программы в набор.Это дает возможность использовать группу тестов как один,что помогает тестировщикам,например в ситуациях,когда разные тесты пишут разные люди.
7. **Selenide** : Помогает делать стабильные тесты,решая(почти) все проблемы с таймаутами и аяксом.Selenide предлагает лаконичный API для использования Selenium WebDriver в UI тестах:Умные ожидания Автоматическое управление браузером(открытие,закрытие-больше не придётся об этом думать.
8. **Lombok** : Упрощает написание кода,позволяя сосредоточиться только на том,что нужно реализовать,а именно:переводе бизнес-требований в код.
9. **Allure** : Предоставляет набор инструментов для построения понятной и прозрачной автоматизации с нуля:от написания теста до аналитики по результатам десятков прогонов.
10. **JavaFaker** :  Библиотека, которую можно использовать для создания и генерации широкого массива случайных данных для тестов.
